...
def parse_medical_document(pdf_path: str, chunk_size: int = 800, overlap: int = 120) -> Dict[str, Any]:
    ...
    return {"source": {"file_name": Path(pdf_path).name, "path": str(Path(pdf_path).resolve()), "source_type": "pdf", "page_count": page_count}, "content": {"raw_text": text, "pages": pages_data, "chunks": chunks, "chunk_details": build_chunk_details(chunks, pages_data, text), "tables": tables}, "metadata": build_metadata(Path(pdf_path).name, page_count, len(chunks), len(tables), "pdf")}